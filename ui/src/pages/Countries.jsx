...
const Countries = () => {
  ...
  // ✅ Fetch Data: Either Random or Filtered
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setAllDishes(res.data?.allDishes || []);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-animate">
      ...
      <div className="min-h-screen p-6 bg-gray-100 bg-animate mt-20 flex-grow py-10">
        <h1 className="text-4xl font-bold text-center mb-6 text-white">Cuisine Explorer</h1>

        <div className="flex flex-col md:flex-row justify-center gap-12 mb-8">
          <CountrySelector selectedCountry={selectedCountry} onChangeCountry={setSelectedCountry} />
          <CategorySelector selectedCategory={selectedCategory} onChangeCategory={setSelectedCategory} />
        </div>
        ...
        {/* ✅ Always show dishes (random or filtered) */}
        {!loading && !error && (
